...
from higgstautau.mixins import TauFourMomentum, ElectronFourMomentum, MCParticle, FourMomentum
from higgstautau.lephad.filters import tau_skimselection, muon_skimselection, electron_skimselection, \
                                       OverlapCheck, PrepareInputTree, VBFFilter, VBFFilter3
...
#        outtree.set_buffer(intree.buffer, create_branches=True, visible=False)
        ...
        intree.define_collection(name="truthjets", prefix="jet_antikt4truth_", size="jet_antikt4truth_n", mix=FourMomentum)
        intree.define_object(name='isLTT', prefix='')
        intree.define_object(name='leptonType', prefix='')

        # set the event filters

        eventFilterList = [PrepareInputTree(), VBFFilter(4.0, 600)]
