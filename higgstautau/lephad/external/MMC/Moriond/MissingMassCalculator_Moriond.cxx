...
#include <iostream>
#include <fstream>
#include "MissingMassCalculator_Moriond.h"
#include <TRandom.h>
#include <TObject.h>
...
MissingMassCalculator::MissingMassCalculator(std::string JERProviderFile){
  ...
  fApplyMassScale=1; // apply mass scale correction by default
  ...
  myJER=new JERProviderMMCMoriond("AntiKt4TopoJES","Truth",JERProviderFile);  
  myJER->init();
...
void MissingMassCalculator::SetMetScanParamsUE(double sumEt, double phi_scan, int data_code) {
  ...
  double METresScale=0.48; // using GCW calibration scheme
  if(data_code==1) METresScale=0.50;
  ...
  sigma= sumEt>5.0 ? METresScale*sqrt(sumEt) : METresScale*sqrt(5.0); // resolution for data, GCW-calibration scheme
...
void MissingMassCalculator::FinalizeInputStuff(InputInfoStuff &fStuff) {
  ...
  if(fStuff.vistau1.P()>3000.0 || fStuff.vistau2.P()>3000.0)
    {
      double scale=1000.0;
...
//       JERProviderMMCMoriond myJER("AntiKt4TopoJES","Truth","JERProviderMMCMoriondPlots.root");
//       myJER.init();
      double pt=fStuff.jet4vecs[0].Pt(); // range [10,5000] in JERProviderMMCMoriond-00-00-02
      double eta=fabs(fStuff.jet4vecs[0].Eta()); // |eta|<4.5 in JERProviderMMCMoriond-00-00-02
...
// 	  uncert = myJER.getUncert(pt,eta);
      ...
      for(unsigned int i=1; i<fStuff.jet4vecs.size(); i++) // start from 2nd jet
	{
	  ...
	  pt=fStuff.jet4vecs[i].Pt(); // range [10,5000] in JERProviderMMCMoriond-00-00-02
	  eta=fabs(fStuff.jet4vecs[i].Eta()); // |eta|<4.5 in JERProviderMMCMoriond-00-00-02
...
int MissingMassCalculator::TailCleanUp(int type1, TLorentzVector vis1, TLorentzVector nu1, 
                                       int type2, TLorentzVector vis2, TLorentzVector nu2, 
                                       double mmc_mass, double vis_mass, double eff_mass) {
  int pass_code=1;
...
//       if(MrecoMeff>2.2) return 0;
  ...
  if((type1==0 || type2==0) && (type1+type2>0)) // lepton-hadron channel
    {
      double MrecoMvis=mmc_mass/vis_mass;
//       if((type1==1 || type2==1) && MrecoMvis>2.5) return 0; // for lep + 1-prong
...
double MissingMassCalculator::TauProbability(int type1, TLorentzVector vis1, TLorentzVector nu1, 
					     int type2, TLorentzVector vis2, TLorentzVector nu2) {
  double prob=1.0;
  double prob1=1.0;
  double prob2=1.0;
