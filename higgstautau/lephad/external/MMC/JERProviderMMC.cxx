...
// #include "JetResolution/JERProviderMMC.h"
#include "JERProviderMMC.h" // modified by Sasha, to run in local directory

////------ ClassImp added by Sasha
//#if !defined(__CINT__) | defined(__MAKECINT__)
//ClassImp(JERProviderMMC);
//#endif
...
JERProviderMMC::JERProviderMMC(std::string CollectionName, std::string MethodName, std::string FileName, int verbose):
  m_collectionName(CollectionName), m_methodName(MethodName), m_fileName(FileName), m_isInit(false), m_verbose(verbose)
{
	//Nothing needed here
}

void JERProviderMMC::setFileName(std::string fileName)
{

  if(!m_isInit) m_fileName = fileName;
  else cout << "ERROR: Input file cannot be changed once JERProviderMMC is initialized!" << endl;
}

void JERProviderMMC::init()
{
  ...
  if(!m_isInit) {
  	...
  	if(!m_inputFile)
    	{
    		...
    		if(m_verbose==1) cout << "JERProviderMMC initialized:  Collection name = " << m_collectionName.c_str() << endl;
  	...
  	if(m_verbose==1) cout << "JERProviderMMC already initialized!" << endl;
  }
}

JERProviderMMC::~JERProviderMMC()
{
...
void JERProviderMMC::setInputCollection(std::string CollectionName, std::string MethodName)
{
...
TF1* JERProviderMMC::getParam(double y)
{
  
  if(!m_isInit) {
    cout << "JERProviderMMC not initialized." << endl;
    return 0;
  }
  ...
  if (y <= 0.8) {
    ...
    if(m_verbose==1) cout << "WARNING: Y outside of covered range (0.0-4.5): Returning parametrization for 4.5" << endl;
    return m_jerFunc[5];
  }
...
TGraphErrors* JERProviderMMC::getOffset(double y)
{
...
TGraphErrors* JERProviderMMC::getSyst(double y)
{
...
float JERProviderMMC::getOffset(double pt, double y) 
{
...
float JERProviderMMC::getRelResolutionMC(double pt, double y) 
{
...
float JERProviderMMC::getRelResolutionData(double pt, double y) {
...
float JERProviderMMC::getResolutionUncert(double pt, double y)
{
...
float JERProviderMMC::getSmearingFactorMC(double pt, double y) {
...
float JERProviderMMC::getSmearingFactorSyst(double pt, double y) {
