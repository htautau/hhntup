...
#ifndef MYROOT_Root_TMsgLogger
#define MYROOT_Root_TMsgLogger
...
namespace External
{
   ...
   public:
      ...
      TMsgLogger( const TObject* source, AnalysisFramework::External::TMsgLevel minLevel = kINFO );
      TMsgLogger( const std::string& source, AnalysisFramework::External::TMsgLevel minLevel = kINFO );
      TMsgLogger( AnalysisFramework::External::TMsgLevel minLevel = kINFO );
      ...
      AnalysisFramework::External::TMsgLevel          GetMinLevel() const     { return m_minLevel; }
      const std::string& GetMinLevelStr() const  { return m_levelMap.find( m_minLevel )->second; }

      AnalysisFramework::External::TMsgLevel   MapLevel( const TString& instr ) const;
      ...
      TMsgLogger& operator<< ( AnalysisFramework::External::TMsgLevel level );
      ...
      static void SetMinLevel( AnalysisFramework::External::TMsgLevel minLevel ) { m_minLevel = minLevel; }

   private:

      // the current minimum level is global for the whole Root
      // it can only be changed by the central storage singleton object
      static AnalysisFramework::External::TMsgLevel                 m_minLevel;       // minimum level for logging output

      // private utility routines
      void Send();
      void InitMaps();
      void WriteMsg( AnalysisFramework::External::TMsgLevel level, const std::string& line ) const;
      ...
      AnalysisFramework::External::TMsgLevel                        m_activeLevel;    // active level
      const std::string::size_type     m_maxSourceSize;  // maximum length of source name

      std::map<AnalysisFramework::External::TMsgLevel, std::string> m_levelMap;       // matches output levels with strings
      std::map<AnalysisFramework::External::TMsgLevel, std::string> m_colorMap;       // matches output levels with terminal colors
   ...
   inline TMsgLogger& TMsgLogger::operator<< ( AnalysisFramework::External::TMsgLevel level ) 
   {
      m_activeLevel = level;
      return *this;
   }
