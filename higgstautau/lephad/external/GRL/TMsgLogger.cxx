...
AnalysisFramework::External::TMsgLevel TMsgLogger::m_minLevel = kINFO;

TMsgLogger::TMsgLogger( const TObject* source, AnalysisFramework::External::TMsgLevel /* minLevel */ )
   : m_objSource( source ), 
     m_strSource( "" ), 
     m_prefix( PREFIX ), 
     m_suffix( SUFFIX ), 
...
TMsgLogger::TMsgLogger( const string& source, AnalysisFramework::External::TMsgLevel /* minLevel */ )
   : m_objSource( 0 ),
     m_strSource( source ), 
     m_prefix( PREFIX ), 
     m_suffix( SUFFIX ), 
...
TMsgLogger::TMsgLogger( AnalysisFramework::External::TMsgLevel /* minLevel */ )
   : m_objSource( 0 ), 
     m_strSource( "Unknown" ), 
     m_prefix( PREFIX ), 
     m_suffix( SUFFIX ), 
...
void TMsgLogger::WriteMsg( AnalysisFramework::External::TMsgLevel mlevel, const std::string& line ) const 
{
   if (mlevel < GetMinLevel()) return;
   map<AnalysisFramework::External::TMsgLevel, std::string>::const_iterator slevel;
...
AnalysisFramework::External::TMsgLevel TMsgLogger::MapLevel( const TString& instr ) const
{
   TString ins = instr; // need to copy
   ins.ToUpper();

   // find the corresponding key
   std::map<AnalysisFramework::External::TMsgLevel, std::string>::const_iterator it = m_levelMap.begin();
