/*
OneSidedFrequentistUpperLimitWithBands_lowCount
...
v3. May, 2010 (uses 5.29 to fix global obs for simpdf)
  -- new low-count variation
  -- update power constraint to 50%
v4. July, 2010 (uses 5.30, streamlined generation of b-only toys)
...
void OneSidedFrequentistUpperLimitWithBands_lowCount(const char* infile = "",
  ...
  int nToyMC = 50; // number of toys used to define the expected limit and band
  ...
  int nworkers = 5;
  ProofConfig pc(*w, nworkers, "",false);  // put in a proof server in the string if you have one
  ...
  cout << "obsTestStat = " << fc.GetTestStatSampler()->EvaluateTestStatistic(*data,tmpPOI) << endl;
  ...
  for(int imc=0; imc<nToyMC; ++imc){
    ...
    // Now get limit.  This is the low-count script, so we repeat the full procedure!

    // make a new FC as before
    FeldmanCousins fc2(*toyData,*mc);
    fc2.SetConfidenceLevel(confidenceLevel); 
    fc2.AdditionalNToysFactor(additionalToysFac); // improve sampling that defines confidence belt
    fc2.UseAdaptiveSampling(true); // speed it up a bit, but don't use for expectd limits
    fc2.SetNBins(nPointsToScan); // set how many points per parameter of interest to scan

    toymcsampler = (ToyMCSampler*) fc2.GetTestStatSampler(); 
    testStat = dynamic_cast<ProfileLikelihoodTestStat*>(toymcsampler->GetTestStatistic());
    testStat->SetOneSided(true);
    
    if(!mc->GetPdf()->canBeExtended()){
      if(data->numEntries()==1)     
	fc2.FluctuateNumDataEntries(false);
      else
	cout <<"Not sure what to do about this model" <<endl;
    }
    ...
    // Now get the interval
    PointSetInterval* binterval = fc2.GetInterval();
    
    // print out the iterval on the first Parameter of Interest
    cout << "\n95% interval on " <<firstPOI->GetName()<<" is : ["<<
      binterval->LowerLimit(*firstPOI) << ", "<<
      binterval->UpperLimit(*firstPOI) <<"] "<<endl;
    
    double thisUL = binterval->UpperLimit(*firstPOI) ;
    histOfUL->Fill(thisUL);

    delete binterval;        
    delete toyData;
    ProofConfig::CloseProof(); // may not be necessary
  ...
  cout <<"time for observed limit"<<endl;
  t.Print();
  cout <<"time for bands"<<endl;
  t2.Print();
